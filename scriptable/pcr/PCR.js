...
class Widget extends Base {
  ...
  async render() {
    ...
    // }
    // notify('widgetFamily', this.widgetFamily)
  ...
  async accessoryInlineWidget({ url, data }) {
    ...
    if (data.expireTimestamp) {
      ...
      subtitleText = cell.addDate(dateFormatter.date(DATE))
      subtitleText.applyRelativeStyle()
    } else {
      subtitleText = cell.addText(data.msg.subtitle)
    }

    // if (data.version) {
    //   cell.addSpacer()
    //   cell.addText(data.version)
    // }
  ...
  async smallWidget({ url, data }) {
    ...
    const nowText = w.addText(
      [
        data.version,
        '刷新于',
        [new Date().getHours(), new Date().getMinutes()].map(i => String(i).padStart(2, '0')).join(':'),
      ]
        .filter(i => i !== null)
        .join(' ')
    )
