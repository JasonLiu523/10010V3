...
!(async () => {
  ...
  try {
    console.log(`开始 删除 Gist ${gist ? gist.id : ''}`)
    const post = opts =>
    ...
    console.log(e)
    throw new Error(`删除 Gist 失败: ${$.lodash_get(e, 'message') || e}`)
  }

  $.msg(`Gist 删除备份`, `✅`, `原 URL ${gist.url} 应该无了`)
})()
  .catch(e => {
    console.log(e)
    $.msg(`Gist 删除备份`, `❌`, `${$.lodash_get(e, 'message') || e}`)
  })
  .finally(() => {
    $.done()
  })
